...
class SensorExternalTaskTest(unittest.TestCase):
    ...
    def test_sensor_external_task(self):
        ...
        def check_depend_itself(current_dag, current_task):
            ...
            if current_task.external_dag_id == current_dag and current_task.external_task_id == current_task.task_id:
        ...
        def check_external_dag_availability(current_task, dags_available):
            ...
            if current_task.external_dag_id not in dags_available:
                raise Exception('DAG named {} is not available'.format(current_task.external_dag_id))


        def check_external_task_availability(current_task, dags_available, dag_files):
            """ Prevent the sensor to depend on unavailable task within a DAG """
            dag_tasks = dag_files[dags_available.index(current_task.external_dag_id)]['instance']['tasks']
            ...
            if current_task.external_task_id not in tasks_list:
                raise Exception('Task named {} is not available in DAG {}'
                                .format(current_task.external_task_id, current_task.external_dag_id))
        ...
        def check_valid_execution_value(current_task, external_task):
            """ Check whether the specified execution date could be achieved by the external task """
            # TO-DO: need to find formula to check whether two arithmetic progression is within another (not only intersect)
            None
